...
class AdminController extends CI_Controller
{
	...
	function send_mail2($customer_id, $password, $pdf, $email)
	{
		...
		$fromemail = "support@theportalbusines.com";
		$toemail = $email;
		...
		$this->email->from($fromemail, "Theportalbusines");
	...
	function send_mail($customer_id, $password, $pdf, $email)
	{
		...
		try {
			...
			$mail->Host   ='182.181.62.50.host.secureserver.net';                     //Set the SMTP server to send through
			$mail->SMTPAuth   = true;                                   //Enable SMTP authentication
			$mail->Username   = 'support@theportalbusines.com';                     //SMTP username
			$mail->Password   = 'cgSSgOsvWe)m';
			...
			//Recipients
			$mail->setFrom('support@theportalbusines.com', 'Theportalbusines');
			$mail->addAddress($email);     //Add a recipient
			$audio_file = '/home/ltx8i4fswi8y/theportalbusines.com/admin_assets/audio_demo.mp3';
			$attactfile = '/home/ltx8i4fswi8y/theportalbusines.com/admin_assets/pdf/' . $pdf;
	...
	function sendmail($email, $mesg, $subject, $attactfile, $fromemail)
	{
		...
		$this->email->initialize($config);
		$this->email->to($toemail);
		$this->email->from($fromemail, "onlineplacement");
		$this->email->subject($subject);
	...
	function remider_mail_send()
	{
		...
		$sendfrom = 'legal@theportalbusines.com';
		$attactfile = ""; //temp
		foreach ($ids as $cid) {
	...
	public function send_mail_withmailer($email, $sendfrom, $mesg, $subject)
	{
		...
		try {
			...
			//$mail->SMTPDebug = SMTP::DEBUG_SERVER;                      //Enable verbose debug output
			$mail->isSMTP();                                            //Send using SMTP
			$mail->Host       = 'mail.theportalbusines.com';                     //Set the SMTP server to send through
			$mail->SMTPAuth   = true;                                   //Enable SMTP authentication

			$mail->Username   = 'legal@theportalbusines.com';                     //SMTP username
			$mail->Password   = 'kIBEJ8tQHg@v';
			...
			$mail->setFrom($sendfrom, 'Theportalbusines');
			$mail->addAddress($email);     //Add a recipient
			$mail->isHTML(true);                                  //Set email format to HTML
	...
	public function autoformapproved()
	{
		...
		foreach($records as $rows){
			...
			if($rows->first_name == $recorddata[0]->first_name){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->last_name == $recorddata[0]->last_name){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->contact_no == $recorddata[0]->contact_no){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->alternate_no == $recorddata[0]->alternate_no){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->email == $recorddata[0]->email){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->company_name == $recorddata[0]->company_name){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->website_url == $recorddata[0]->website_url){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->address == $recorddata[0]->address){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->city == $recorddata[0]->city){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->state == $recorddata[0]->state){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->sic_desc == $recorddata[0]->sic_desc){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->sic_code == $recorddata[0]->sic_code){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->entity_type == $recorddata[0]->entity_type){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->company_sale == $recorddata[0]->company_sale){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->revenue == $recorddata[0]->revenue){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->country == $recorddata[0]->country){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
			...
			if($rows->medical_ins == $recorddata[0]->medical_ins){
				$fid_status = '1';
			} else {
				$fid_status = '0';
			}
	...
	{
		$customer_id = $this->input->post('customer_id');
		$set['submission_status'] = $this->input->post('status');
		$where = array('customer_id' => $customer_id);
		$this->model->hdm_update_where('tbl_register', $set, $where);
		return redirect('customer_qc');
	}
